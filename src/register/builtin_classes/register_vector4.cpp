...
static void vector4_class_finalizer(JSRuntime *rt, JSValue val) {
	...
	Vector4 *vector4 = static_cast<Vector4 *>(JS_GetOpaque(val, class_id));
	if (vector4)
		memfree(vector4);
}
...
void define_vector4_constants(JSContext *ctx, JSValue ctor) {
	...
	JS_DefinePropertyValueStr(ctx, ctor, "ZERO", VariantAdapter(Vector4(0, 0, 0, 0)), JS_PROP_ENUMERABLE);
	JS_DefinePropertyValueStr(ctx, ctor, "ONE", VariantAdapter(Vector4(1, 1, 1, 1)), JS_PROP_ENUMERABLE);
	JS_DefinePropertyValueStr(ctx, ctor, "INF", VariantAdapter(Vector4(INFINITY, INFINITY, INFINITY, INFINITY)), JS_PROP_ENUMERABLE);
}
