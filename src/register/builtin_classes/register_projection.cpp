...
static void projection_class_finalizer(JSRuntime *rt, JSValue val) {
	...
	Projection *projection = static_cast<Projection *>(JS_GetOpaque(val, class_id));
	if (projection)
		memfree(projection);
}
...
static JSValue projection_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 4 &&VariantAdapter(argv[0]).get_type() == Variant::Type::VECTOR4&&VariantAdapter(argv[1]).get_type() == Variant::Type::VECTOR4&&VariantAdapter(argv[2]).get_type() == Variant::Type::VECTOR4&&VariantAdapter(argv[3]).get_type() == Variant::Type::VECTOR4) {
		...
		Vector4 v1 = VariantAdapter(argv[1]);
		Vector4 v2 = VariantAdapter(argv[2]);
		Vector4 v3 = VariantAdapter(argv[3]);
...
static JSValue projection_class_set_z(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	Projection &val = *reinterpret_cast<Projection *>(JS_GetOpaque(this_val, classes["Projection"]));
	val.columns->z = VariantAdapter(*argv);
	return JS_UNDEFINED;
}
...
static JSValue projection_class_set_w(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	Projection &val = *reinterpret_cast<Projection *>(JS_GetOpaque(this_val, classes["Projection"]));
	val.columns->w = VariantAdapter(*argv);
	return JS_UNDEFINED;
}
...
void define_projection_constants(JSContext *ctx, JSValue ctor) {
	JS_DefinePropertyValueStr(ctx, ctor, "PLANE_NEAR", VariantAdapter(0), JS_PROP_ENUMERABLE);
	JS_DefinePropertyValueStr(ctx, ctor, "PLANE_FAR", VariantAdapter(1), JS_PROP_ENUMERABLE);
	JS_DefinePropertyValueStr(ctx, ctor, "PLANE_LEFT", VariantAdapter(2), JS_PROP_ENUMERABLE);
	JS_DefinePropertyValueStr(ctx, ctor, "PLANE_TOP", VariantAdapter(3), JS_PROP_ENUMERABLE);
	JS_DefinePropertyValueStr(ctx, ctor, "PLANE_RIGHT", VariantAdapter(4), JS_PROP_ENUMERABLE);
	JS_DefinePropertyValueStr(ctx, ctor, "PLANE_BOTTOM", VariantAdapter(5), JS_PROP_ENUMERABLE);
	JS_DefinePropertyValueStr(ctx, ctor, "IDENTITY", VariantAdapter(Projection(Vector4(1, 0, 0, 0), Vector4(0, 1, 0, 0), Vector4(0, 0, 1, 0), Vector4(0, 0, 0, 1))), JS_PROP_ENUMERABLE);
	JS_DefinePropertyValueStr(ctx, ctor, "ZERO", VariantAdapter(Projection(Vector4(0, 0, 0, 0), Vector4(0, 0, 0, 0), Vector4(0, 0, 0, 0), Vector4(0, 0, 0, 0))), JS_PROP_ENUMERABLE);
}
