...
static void vector2_class_finalizer(JSRuntime *rt, JSValue val) {
	...
	Vector2 *vector2 = static_cast<Vector2 *>(JS_GetOpaque(val, class_id));
	if (vector2)
		memfree(vector2);
}
...
void define_vector2_constants(JSContext *ctx, JSValue ctor) {
	...
	JS_DefinePropertyValueStr(ctx, ctor, "ZERO", VariantAdapter(Vector2(0, 0)), JS_PROP_ENUMERABLE);
	JS_DefinePropertyValueStr(ctx, ctor, "ONE", VariantAdapter(Vector2(1, 1)), JS_PROP_ENUMERABLE);
	JS_DefinePropertyValueStr(ctx, ctor, "INF", VariantAdapter(Vector2(INFINITY, INFINITY)), JS_PROP_ENUMERABLE);
	JS_DefinePropertyValueStr(ctx, ctor, "LEFT", VariantAdapter(Vector2(-1, 0)), JS_PROP_ENUMERABLE);
	JS_DefinePropertyValueStr(ctx, ctor, "RIGHT", VariantAdapter(Vector2(1, 0)), JS_PROP_ENUMERABLE);
	JS_DefinePropertyValueStr(ctx, ctor, "UP", VariantAdapter(Vector2(0, -1)), JS_PROP_ENUMERABLE);
	JS_DefinePropertyValueStr(ctx, ctor, "DOWN", VariantAdapter(Vector2(0, 1)), JS_PROP_ENUMERABLE);
}
...
static int js_vector2_class_init(JSContext *ctx) {
	...
	define_vector2_constants(ctx, ctor);
