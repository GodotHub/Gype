...
void define_packed_string_array_property(JSContext *ctx, JSValue obj) {
}

static int js_packed_string_array_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	define_packed_string_array_property(ctx, proto);

	JSValue ctor = JS_NewCFunction2(ctx, packed_string_array_class_constructor, "PackedStringArray", 0, JS_CFUNC_constructor, 0);
	JS_SetConstructor(ctx, ctor, proto);

	JS_SetModuleExport(ctx, m, "PackedStringArray", ctor);

	return 0;
}

JSModuleDef *_js_init_packed_string_array_module(JSContext *ctx, const char *module_name) {
	JSModuleDef *m = JS_NewCModule(ctx, module_name, js_packed_string_array_class_init);
	if (!m)
		return NULL;
	JS_AddModuleExport(ctx, m, "PackedStringArray");
	return m;
}

JSModuleDef *js_init_packed_string_array_module(JSContext *ctx) {
	return _js_init_packed_string_array_module(ctx, "godot/classes/packed_string_array");
}
