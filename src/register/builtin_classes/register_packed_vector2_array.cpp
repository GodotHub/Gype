...
static void packed_vector2_array_class_finalizer(JSRuntime *rt, JSValue val) {
	...
	PackedVector2Array *packed_vector2_array = static_cast<PackedVector2Array *>(JS_GetOpaque(val, class_id));
...
static JSValue packed_vector2_array_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	PackedVector2Array *packed_vector2_array_class = nullptr;
...
static int js_packed_vector2_array_class_init(JSContext *ctx) {
	...
	JS_SetPropertyFunctionList(ctx, proto, packed_vector2_array_class_proto_funcs, _countof(packed_vector2_array_class_proto_funcs));
