...
static void polygon_occluder3d_class_finalizer(JSRuntime *rt, JSValue val) {
	PolygonOccluder3D *polygon_occluder3d = static_cast<PolygonOccluder3D *>(JS_GetOpaque(val, PolygonOccluder3D::__class_id));
	if (polygon_occluder3d)
		PolygonOccluder3D::free(nullptr, polygon_occluder3d);
}
...
static JSValue polygon_occluder3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	JSValue obj = JS_NewObjectClass(ctx, PolygonOccluder3D::__class_id);
...
static JSValue polygon_occluder3d_class_set_polygon(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	call_builtin_method_no_ret(&PolygonOccluder3D::set_polygon, PolygonOccluder3D::__class_id, ctx, this_val, argv);
	return JS_UNDEFINED;
};
static JSValue polygon_occluder3d_class_get_polygon(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	return call_builtin_const_method_ret(&PolygonOccluder3D::get_polygon, PolygonOccluder3D::__class_id, ctx, this_val, argv);
};
