...
static JSValue visual_shader_node_multiply_add_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (JS_IsException(obj))
		return obj;
	VisualShaderNodeMultiplyAdd *visual_shader_node_multiply_add_class = memnew(VisualShaderNodeMultiplyAdd);
...
static JSValue visual_shader_node_multiply_add_class_set_op_type(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    call_builtin_method_no_ret(&VisualShaderNodeMultiplyAdd::set_op_type, ctx, this_val, argc, argv);
	return JS_UNDEFINED;
};
...
void define_visual_shader_node_multiply_add_property(JSContext *ctx, JSValue obj) {
    JS_DefinePropertyGetSet(
        ctx,
        obj,
        JS_NewAtom(ctx, "op_type"),
        JS_NewCFunction(ctx, visual_shader_node_multiply_add_class_get_op_type, "get_op_type", 0),
        JS_NewCFunction(ctx, visual_shader_node_multiply_add_class_set_op_type, "set_op_type", 0),
