...
static JSValue graph_frame_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		graph_frame_class = static_cast<GraphFrame *>(Variant(*argv).operator Object *());
	else 
...
static void define_graph_frame_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_graph_frame_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	define_graph_frame_enum(ctx, proto);
	...
	constructors[GraphFrame::__class_id] = ctor;
