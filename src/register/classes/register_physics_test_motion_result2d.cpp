...
static JSValue physics_test_motion_result2d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		physics_test_motion_result2d_class = static_cast<PhysicsTestMotionResult2D *>(Variant(*argv).operator Object *());
	else 
...
static void define_physics_test_motion_result2d_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_physics_test_motion_result2d_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	define_physics_test_motion_result2d_enum(ctx, proto);
	...
	constructors[PhysicsTestMotionResult2D::__class_id] = ctor;
