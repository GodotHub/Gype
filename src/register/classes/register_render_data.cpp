...
static JSValue render_data_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		render_data_class = static_cast<RenderData *>(Variant(*argv).operator Object *());
	else 
...
static void define_render_data_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_render_data_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	define_render_data_enum(ctx, proto);
	...
	constructors[RenderData::__class_id] = ctor;
