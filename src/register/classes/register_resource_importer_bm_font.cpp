...
static JSValue resource_importer_bm_font_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		resource_importer_bm_font_class = static_cast<ResourceImporterBMFont *>(Variant(*argv).operator Object *());
	else 
...
static void define_resource_importer_bm_font_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_resource_importer_bm_font_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	define_resource_importer_bm_font_enum(ctx, proto);
	...
	constructors[ResourceImporterBMFont::__class_id] = ctor;
