...
static JSValue physics_shape_query_parameters3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		physics_shape_query_parameters3d_class = static_cast<PhysicsShapeQueryParameters3D *>(Variant(*argv).operator Object *());
	else 
...
static void define_physics_shape_query_parameters3d_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_physics_shape_query_parameters3d_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	define_physics_shape_query_parameters3d_enum(ctx, proto);
	...
	constructors[PhysicsShapeQueryParameters3D::__class_id] = ctor;
