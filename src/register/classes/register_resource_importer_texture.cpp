...
static JSValue resource_importer_texture_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) {
		Variant vobj = *argv;
		resource_importer_texture_class = static_cast<ResourceImporterTexture *>(static_cast<Object *>(vobj));
	} else {
...
void define_resource_importer_texture_property(JSContext *ctx, JSValue obj) {
}
...
void register_resource_importer_texture() {
	ResourceImporterTexture::__init_js_class_id();
