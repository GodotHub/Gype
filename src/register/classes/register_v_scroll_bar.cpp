...
static JSValue v_scroll_bar_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	JSValue obj = JS_NewObjectProtoClass(ctx, proto, VScrollBar::__class_id);
	if (JS_IsException(obj))
		return obj;
	VScrollBar *v_scroll_bar_class = memnew(VScrollBar);
...
static int js_v_scroll_bar_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	JSValue proto = JS_NewObjectClass(ctx, VScrollBar::__class_id);
