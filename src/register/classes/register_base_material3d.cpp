...
static void define_base_material3d_enum(JSContext *ctx, JSValue proto) {
	...
	JS_SetPropertyStr(ctx, TextureParam_obj, "TEXTURE_MAX", JS_NewInt64(ctx, 18));
	...
	JS_SetPropertyStr(ctx, Feature_obj, "FEATURE_MAX", JS_NewInt64(ctx, 12));
	...
	JS_SetPropertyStr(ctx, Flags_obj, "FLAG_MAX", JS_NewInt64(ctx, 22));
...
static int js_base_material3d_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	constructors[BaseMaterial3D::__class_id] = ctor;
