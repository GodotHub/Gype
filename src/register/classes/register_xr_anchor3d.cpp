...
static JSValue xr_anchor3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		xr_anchor3d_class = static_cast<XRAnchor3D *>(Variant(*argv).operator Object *());
	else 
...
static int js_xr_anchor3d_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	JSValue proto = JS_NewObjectClass(ctx, XRAnchor3D::__class_id);
	...
	constructors[XRAnchor3D::__class_id] = ctor;
