...
static JSValue audio_effect_delay_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		audio_effect_delay_class = static_cast<AudioEffectDelay *>(Variant(*argv).operator Object *());
	else 
...
static int js_audio_effect_delay_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	JSValue proto = JS_NewObjectClass(ctx, AudioEffectDelay::__class_id);
	...
	constructors[AudioEffectDelay::__class_id] = ctor;
