...
static void visual_shader_node_vector_compose_class_finalizer(JSRuntime *rt, JSValue val) {
	VisualShaderNodeVectorCompose *visual_shader_node_vector_compose = static_cast<VisualShaderNodeVectorCompose *>(JS_GetOpaque(val, VisualShaderNodeVectorCompose::__class_id));
	if (visual_shader_node_vector_compose)
		VisualShaderNodeVectorCompose::free(nullptr, visual_shader_node_vector_compose);
}
...
static JSValue visual_shader_node_vector_compose_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	JSValue obj = JS_NewObjectClass(ctx, VisualShaderNodeVectorCompose::__class_id);
...
void define_visual_shader_node_vector_compose_property(JSContext *ctx, JSValue obj) {
}

static int js_visual_shader_node_vector_compose_class_init(JSContext *ctx, JSModuleDef *m) {
	
	JS_NewClassID(&VisualShaderNodeVectorCompose::__class_id);
