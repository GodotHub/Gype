...
static JSValue editor_translation_parser_plugin_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		editor_translation_parser_plugin_class = static_cast<EditorTranslationParserPlugin *>(Variant(*argv).operator Object *());
	else 
...
static int js_editor_translation_parser_plugin_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	JSValue proto = JS_NewObjectClass(ctx, EditorTranslationParserPlugin::__class_id);
	...
	constructors[EditorTranslationParserPlugin::__class_id] = ctor;
