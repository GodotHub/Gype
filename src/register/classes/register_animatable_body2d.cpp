...
static JSValue animatable_body2d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		animatable_body2d_class = static_cast<AnimatableBody2D *>(Variant(*argv).operator Object *());
	else 
...
static void define_animatable_body2d_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_animatable_body2d_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	define_animatable_body2d_enum(ctx, proto);
	...
	constructors[AnimatableBody2D::__class_id] = ctor;
