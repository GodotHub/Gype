...
static JSValue packed_data_container_ref_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		packed_data_container_ref_class = static_cast<PackedDataContainerRef *>(Variant(*argv).operator Object *());
	else 
...
static void define_packed_data_container_ref_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_packed_data_container_ref_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	define_packed_data_container_ref_enum(ctx, proto);
	...
	constructors[PackedDataContainerRef::__class_id] = ctor;
