...
static JSValue resource_importer_obj_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	JSValue obj = JS_NewObjectProtoClass(ctx, proto, ResourceImporterOBJ::__class_id);
	if (JS_IsException(obj))
		return obj;

	ResourceImporterOBJ *resource_importer_obj_class;
	if (argc == 1) 
		resource_importer_obj_class = static_cast<ResourceImporterOBJ *>(Variant(*argv).operator Object *());
	else 
...
static void define_resource_importer_obj_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_resource_importer_obj_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	JSValue proto = JS_NewObjectClass(ctx, ResourceImporterOBJ::__class_id);
	...
	define_resource_importer_obj_enum(ctx, proto);
	...
	constructors[ResourceImporterOBJ::__class_id] = ctor;
