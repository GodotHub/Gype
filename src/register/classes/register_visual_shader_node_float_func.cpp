...
static void visual_shader_node_float_func_class_finalizer(JSRuntime *rt, JSValue val) {
	...
	if (visual_shader_node_float_func)
		memdelete(visual_shader_node_float_func);
}
...
static JSValue visual_shader_node_float_func_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	JSValue obj = JS_NewObjectProtoClass(ctx, proto, VisualShaderNodeFloatFunc::__class_id);
	if (JS_IsException(obj))
		return obj;
	VisualShaderNodeFloatFunc *visual_shader_node_float_func_class = memnew(VisualShaderNodeFloatFunc);
...
static JSValue visual_shader_node_float_func_class_set_function(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    call_builtin_method_no_ret(&VisualShaderNodeFloatFunc::set_function, ctx, this_val, argc, argv);
	return JS_UNDEFINED;
};
...
void define_visual_shader_node_float_func_property(JSContext *ctx, JSValue obj) {
    JS_DefinePropertyGetSet(
        ctx,
        obj,
        JS_NewAtom(ctx, "function"),
        JS_NewCFunction(ctx, visual_shader_node_float_func_class_get_function, "get_function", 0),
        JS_NewCFunction(ctx, visual_shader_node_float_func_class_set_function, "set_function", 0),
...
static int js_visual_shader_node_float_func_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	class_id_list.insert(VisualShaderNodeFloatFunc::__class_id);
	JS_NewClass(JS_GetRuntime(ctx), VisualShaderNodeFloatFunc::__class_id, &visual_shader_node_float_func_class_def);

	JSValue proto = JS_NewObjectClass(ctx, VisualShaderNodeFloatFunc::__class_id);
...
void register_visual_shader_node_float_func() {
	VisualShaderNodeFloatFunc::__init_js_class_id();
