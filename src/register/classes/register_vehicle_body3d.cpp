...
static JSValue vehicle_body3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		vehicle_body3d_class = static_cast<VehicleBody3D *>(Variant(*argv).operator Object *());
	else 
...
static void define_vehicle_body3d_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_vehicle_body3d_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	define_vehicle_body3d_enum(ctx, proto);
	...
	constructors[VehicleBody3D::__class_id] = ctor;
