...
static JSValue visual_shader_node_rotation_by_axis_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	JSValue obj = JS_NewObjectProtoClass(ctx, proto, VisualShaderNodeRotationByAxis::__class_id);
	if (JS_IsException(obj))
		return obj;

	VisualShaderNodeRotationByAxis *visual_shader_node_rotation_by_axis_class;
	if (argc == 1) 
		visual_shader_node_rotation_by_axis_class = static_cast<VisualShaderNodeRotationByAxis *>(Variant(*argv).operator Object *());
	else 
...
static int js_visual_shader_node_rotation_by_axis_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	class_id_list.insert(VisualShaderNodeRotationByAxis::__class_id);
	JS_NewClass(JS_GetRuntime(ctx), VisualShaderNodeRotationByAxis::__class_id, &visual_shader_node_rotation_by_axis_class_def);

	JSValue proto = JS_NewObjectClass(ctx, VisualShaderNodeRotationByAxis::__class_id);
	...
	constructors[VisualShaderNodeRotationByAxis::__class_id] = ctor;
...
void register_visual_shader_node_rotation_by_axis() {
	VisualShaderNodeRotationByAxis::__init_js_class_id();
