...
static JSValue input_event_midi_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		input_event_midi_class = static_cast<InputEventMIDI *>(Variant(*argv).operator Object *());
	else 
...
static void define_input_event_midi_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_input_event_midi_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	define_input_event_midi_enum(ctx, proto);
	...
	constructors[InputEventMIDI::__class_id] = ctor;
