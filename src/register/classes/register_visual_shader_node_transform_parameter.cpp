...
static void visual_shader_node_transform_parameter_class_finalizer(JSRuntime *rt, JSValue val) {
	...
	if (visual_shader_node_transform_parameter)
		memdelete(visual_shader_node_transform_parameter);
}
...
static JSValue visual_shader_node_transform_parameter_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	JSValue obj = JS_NewObjectProtoClass(ctx, proto, VisualShaderNodeTransformParameter::__class_id);
	if (JS_IsException(obj))
		return obj;

	VisualShaderNodeTransformParameter *visual_shader_node_transform_parameter_class;
	if (argc == 1) 
		visual_shader_node_transform_parameter_class = static_cast<VisualShaderNodeTransformParameter *>(Variant(*argv).operator Object *());
	else 
...
static JSValue visual_shader_node_transform_parameter_class_get_default_value(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
	return call_builtin_const_method_ret(&VisualShaderNodeTransformParameter::get_default_value, ctx, this_val, argc, argv);
};
...
static int js_visual_shader_node_transform_parameter_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	class_id_list.insert(VisualShaderNodeTransformParameter::__class_id);
	...
	JSValue proto = JS_NewObjectClass(ctx, VisualShaderNodeTransformParameter::__class_id);
	...
	constructors[VisualShaderNodeTransformParameter::__class_id] = ctor;
...
void register_visual_shader_node_transform_parameter() {
	VisualShaderNodeTransformParameter::__init_js_class_id();
