...
static JSValue font_file_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		font_file_class = static_cast<FontFile *>(Variant(*argv).operator Object *());
	else 
...
static void define_font_file_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_font_file_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	define_font_file_enum(ctx, proto);
	...
	constructors[FontFile::__class_id] = ctor;
