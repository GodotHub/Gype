...
static JSValue visual_shader_node_transform_decompose_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	JSValue obj = JS_NewObjectProtoClass(ctx, proto, VisualShaderNodeTransformDecompose::__class_id);
	if (JS_IsException(obj))
		return obj;

	VisualShaderNodeTransformDecompose *visual_shader_node_transform_decompose_class;
	if (argc == 1) 
		visual_shader_node_transform_decompose_class = static_cast<VisualShaderNodeTransformDecompose *>(static_cast<Object *>(Variant(*argv)));
	else 
...
static int js_visual_shader_node_transform_decompose_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	class_id_list.insert(VisualShaderNodeTransformDecompose::__class_id);
	...
	JSValue proto = JS_NewObjectClass(ctx, VisualShaderNodeTransformDecompose::__class_id);
...
void register_visual_shader_node_transform_decompose() {
	VisualShaderNodeTransformDecompose::__init_js_class_id();
