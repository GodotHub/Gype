...
static JSValue vehicle_wheel3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		vehicle_wheel3d_class = static_cast<VehicleWheel3D *>(Variant(*argv).operator Object *());
	else 
...
static void define_vehicle_wheel3d_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_vehicle_wheel3d_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	define_vehicle_wheel3d_enum(ctx, proto);
	...
	constructors[VehicleWheel3D::__class_id] = ctor;
