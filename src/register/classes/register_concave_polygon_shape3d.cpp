...
static void concave_polygon_shape3d_class_finalizer(JSRuntime *rt, JSValue val) {
	...
	if (concave_polygon_shape3d)
		memdelete(concave_polygon_shape3d);
}
...
static JSValue concave_polygon_shape3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		concave_polygon_shape3d_class = static_cast<ConcavePolygonShape3D *>(Variant(*argv).operator Object *());
	else 
...
static void define_concave_polygon_shape3d_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_concave_polygon_shape3d_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	define_concave_polygon_shape3d_enum(ctx, proto);
	...
	constructors[ConcavePolygonShape3D::__class_id] = ctor;
