...
static void audio_effect_low_shelf_filter_class_finalizer(JSRuntime *rt, JSValue val) {
	AudioEffectLowShelfFilter *audio_effect_low_shelf_filter = static_cast<AudioEffectLowShelfFilter *>(JS_GetOpaque(val, AudioEffectLowShelfFilter::__class_id));
	if (audio_effect_low_shelf_filter)
		memdelete(audio_effect_low_shelf_filter);
}
...
static JSValue audio_effect_low_shelf_filter_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		audio_effect_low_shelf_filter_class = static_cast<AudioEffectLowShelfFilter *>(Variant(*argv).operator Object *());
	else 
...
static int js_audio_effect_low_shelf_filter_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	constructors[AudioEffectLowShelfFilter::__class_id] = ctor;
