...
static JSValue animated_sprite2d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		animated_sprite2d_class = static_cast<AnimatedSprite2D *>(Variant(*argv).operator Object *());
	else 
...
static void define_animated_sprite2d_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_animated_sprite2d_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	define_animated_sprite2d_enum(ctx, proto);
	...
	constructors[AnimatedSprite2D::__class_id] = ctor;
