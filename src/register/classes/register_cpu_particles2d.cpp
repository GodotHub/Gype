...
static JSValue cpu_particles2d_class_get_finished_signal(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	CHECK_INSTANCE_VALID_V(this_val);
	CPUParticles2D *opaque = reinterpret_cast<CPUParticles2D *>(JS_GetOpaque(this_val, JS_GetClassID(this_val)));
...
static void define_cpu_particles2d_enum(JSContext *ctx, JSValue proto) {
...
static int js_cpu_particles2d_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	define_cpu_particles2d_enum(ctx, proto);
	...
	constructors[CPUParticles2D::__class_id] = ctor;
