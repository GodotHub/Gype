...
static JSValue navigation_path_query_result2d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		navigation_path_query_result2d_class = static_cast<NavigationPathQueryResult2D *>(Variant(*argv).operator Object *());
	else 
...
static int js_navigation_path_query_result2d_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	JSValue proto = JS_NewObjectClass(ctx, NavigationPathQueryResult2D::__class_id);
	...
	constructors[NavigationPathQueryResult2D::__class_id] = ctor;
