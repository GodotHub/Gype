...
static void importer_mesh_class_finalizer(JSRuntime *rt, JSValue val) {
	ImporterMesh *importer_mesh = static_cast<ImporterMesh *>(JS_GetOpaque(val, ImporterMesh::__class_id));
	if (importer_mesh)
		ImporterMesh::free(nullptr, importer_mesh);
}
...
static JSValue importer_mesh_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	JSValue obj = JS_NewObjectClass(ctx, ImporterMesh::__class_id);
