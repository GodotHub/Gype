...
static JSValue resource_importer_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	JSValue obj = JS_NewObjectProtoClass(ctx, proto, ResourceImporter::__class_id);
	if (JS_IsException(obj))
		return obj;

	ResourceImporter *resource_importer_class;
	if (argc == 1) 
		resource_importer_class = static_cast<ResourceImporter *>(Variant(*argv).operator Object *());
	else 
...
static int js_resource_importer_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	JSValue proto = JS_NewObjectClass(ctx, ResourceImporter::__class_id);
	...
	constructors[ResourceImporter::__class_id] = ctor;
