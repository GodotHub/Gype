...
static JSValue visual_shader_node_u_int_func_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) {
		Variant vobj = *argv;
		visual_shader_node_u_int_func_class = static_cast<VisualShaderNodeUIntFunc *>(static_cast<Object *>(vobj));
	} else {
...
static JSValue visual_shader_node_u_int_func_class_set_function(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    call_builtin_method_no_ret(&VisualShaderNodeUIntFunc::set_function, ctx, this_val, argc, argv);
	return JS_UNDEFINED;
};
...
void define_visual_shader_node_u_int_func_property(JSContext *ctx, JSValue obj) {
