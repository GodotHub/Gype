...
static void visual_shader_node_resizable_base_class_finalizer(JSRuntime *rt, JSValue val) {
	...
	if (visual_shader_node_resizable_base)
		memdelete(visual_shader_node_resizable_base);
}
...
static JSValue visual_shader_node_resizable_base_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		visual_shader_node_resizable_base_class = static_cast<VisualShaderNodeResizableBase *>(Variant(*argv).operator Object *());
	else 
...
static void define_visual_shader_node_resizable_base_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_visual_shader_node_resizable_base_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	define_visual_shader_node_resizable_base_enum(ctx, proto);
	...
	constructors[VisualShaderNodeResizableBase::__class_id] = ctor;
