...
static JSValue array_mesh_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		array_mesh_class = static_cast<ArrayMesh *>(Variant(*argv).operator Object *());
	else 
...
static void define_array_mesh_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_array_mesh_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	define_array_mesh_enum(ctx, proto);
	...
	constructors[ArrayMesh::__class_id] = ctor;
