...
static JSValue gpu_particles_attractor3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		gpu_particles_attractor3d_class = static_cast<GPUParticlesAttractor3D *>(Variant(*argv).operator Object *());
	else 
...
static void define_gpu_particles_attractor3d_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_gpu_particles_attractor3d_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	define_gpu_particles_attractor3d_enum(ctx, proto);
	...
	constructors[GPUParticlesAttractor3D::__class_id] = ctor;
