...
static JSValue visual_shader_node_vector_refract_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) {
		Variant vobj = *argv;
		visual_shader_node_vector_refract_class = static_cast<VisualShaderNodeVectorRefract *>(static_cast<Object *>(vobj));
	} else {
...
void define_visual_shader_node_vector_refract_property(JSContext *ctx, JSValue obj) {
}
