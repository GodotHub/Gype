...
static JSValue input_event_with_modifiers_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		input_event_with_modifiers_class = static_cast<InputEventWithModifiers *>(Variant(*argv).operator Object *());
	else 
...
static int js_input_event_with_modifiers_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	JSValue proto = JS_NewObjectClass(ctx, InputEventWithModifiers::__class_id);
	...
	constructors[InputEventWithModifiers::__class_id] = ctor;
