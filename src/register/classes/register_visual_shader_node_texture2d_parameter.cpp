...
static JSValue visual_shader_node_texture2d_parameter_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		visual_shader_node_texture2d_parameter_class = static_cast<VisualShaderNodeTexture2DParameter *>(Variant(*argv).operator Object *());
	else 
...
static void define_visual_shader_node_texture2d_parameter_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_visual_shader_node_texture2d_parameter_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	define_visual_shader_node_texture2d_parameter_enum(ctx, proto);
	...
	constructors[VisualShaderNodeTexture2DParameter::__class_id] = ctor;
...
void register_visual_shader_node_texture2d_parameter() {
	VisualShaderNodeTexture2DParameter::__init_js_class_id();
