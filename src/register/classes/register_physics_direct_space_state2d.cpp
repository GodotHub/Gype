...
static JSValue physics_direct_space_state2d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) {
		Variant vobj = *argv;
		physics_direct_space_state2d_class = static_cast<PhysicsDirectSpaceState2D *>(static_cast<Object *>(vobj));
	} else {
...
void define_physics_direct_space_state2d_property(JSContext *ctx, JSValue obj) {
}
