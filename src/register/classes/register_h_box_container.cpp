...
static JSValue h_box_container_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		h_box_container_class = static_cast<HBoxContainer *>(Variant(*argv).operator Object *());
	else 
...
static void define_h_box_container_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_h_box_container_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	define_h_box_container_enum(ctx, proto);
	...
	constructors[HBoxContainer::__class_id] = ctor;
