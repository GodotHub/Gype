...
static void visual_shader_node_uv_polar_coord_class_finalizer(JSRuntime *rt, JSValue val) {
	VisualShaderNodeUVPolarCoord *visual_shader_node_uv_polar_coord = static_cast<VisualShaderNodeUVPolarCoord *>(JS_GetOpaque(val, VisualShaderNodeUVPolarCoord::__class_id));
	if (visual_shader_node_uv_polar_coord)
		memdelete(visual_shader_node_uv_polar_coord);
}
...
static JSValue visual_shader_node_uv_polar_coord_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	VisualShaderNodeUVPolarCoord *visual_shader_node_uv_polar_coord_class;
	if (argc == 1) 
		visual_shader_node_uv_polar_coord_class = static_cast<VisualShaderNodeUVPolarCoord *>(Variant(*argv).operator Object *());
	else 
		visual_shader_node_uv_polar_coord_class = memnew(VisualShaderNodeUVPolarCoord);
...
static int js_visual_shader_node_uv_polar_coord_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	constructors[VisualShaderNodeUVPolarCoord::__class_id] = ctor;
