...
static JSValue editor_resource_preview_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		editor_resource_preview_class = static_cast<EditorResourcePreview *>(Variant(*argv).operator Object *());
	else 
...
static void define_editor_resource_preview_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_editor_resource_preview_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	define_editor_resource_preview_enum(ctx, proto);
	...
	constructors[EditorResourcePreview::__class_id] = ctor;
