...
static JSValue visual_shader_node_color_func_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	JSValue obj = JS_NewObjectProtoClass(ctx, proto, VisualShaderNodeColorFunc::__class_id);
	if (JS_IsException(obj))
		return obj;

	VisualShaderNodeColorFunc *visual_shader_node_color_func_class;
	if (argc == 1) 
		visual_shader_node_color_func_class = static_cast<VisualShaderNodeColorFunc *>(Variant(*argv).operator Object *());
	else 
...
static int js_visual_shader_node_color_func_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	class_id_list.insert(VisualShaderNodeColorFunc::__class_id);
	JS_NewClass(JS_GetRuntime(ctx), VisualShaderNodeColorFunc::__class_id, &visual_shader_node_color_func_class_def);

	JSValue proto = JS_NewObjectClass(ctx, VisualShaderNodeColorFunc::__class_id);
	...
	constructors[VisualShaderNodeColorFunc::__class_id] = ctor;
...
void register_visual_shader_node_color_func() {
	VisualShaderNodeColorFunc::__init_js_class_id();
