...
static JSValue audio_sample_playback_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) {
		Variant vobj = *argv;
		audio_sample_playback_class = static_cast<AudioSamplePlayback *>(static_cast<Object *>(vobj));
	} else {
...
void define_audio_sample_playback_property(JSContext *ctx, JSValue obj) {
}
