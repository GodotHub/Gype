...
static JSValue audio_effect_band_limit_filter_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) {
		Variant vobj = *argv;
		audio_effect_band_limit_filter_class = static_cast<AudioEffectBandLimitFilter *>(static_cast<Object *>(vobj));
	} else {
...
void define_audio_effect_band_limit_filter_property(JSContext *ctx, JSValue obj) {
}
