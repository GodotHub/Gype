...
static JSValue input_event_mouse_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		input_event_mouse_class = static_cast<InputEventMouse *>(Variant(*argv).operator Object *());
	else 
...
static void define_input_event_mouse_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_input_event_mouse_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	define_input_event_mouse_enum(ctx, proto);
	...
	constructors[InputEventMouse::__class_id] = ctor;
