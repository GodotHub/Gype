...
static JSValue visual_shader_node_switch_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) {
		Variant vobj = *argv;
		visual_shader_node_switch_class = static_cast<VisualShaderNodeSwitch *>(static_cast<Object *>(vobj));
	} else {
...
static JSValue visual_shader_node_switch_class_set_op_type(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
    ...
    call_builtin_method_no_ret(&VisualShaderNodeSwitch::set_op_type, ctx, this_val, argc, argv);
	return JS_UNDEFINED;
};
...
void define_visual_shader_node_switch_property(JSContext *ctx, JSValue obj) {
