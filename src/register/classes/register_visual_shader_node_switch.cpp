...
static JSValue visual_shader_node_switch_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	JSValue obj = JS_NewObjectProtoClass(ctx, proto, VisualShaderNodeSwitch::__class_id);
	if (JS_IsException(obj))
		return obj;

	VisualShaderNodeSwitch *visual_shader_node_switch_class;
	if (argc == 1) 
		visual_shader_node_switch_class = static_cast<VisualShaderNodeSwitch *>(Variant(*argv).operator Object *());
	else 
...
static int js_visual_shader_node_switch_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	class_id_list.insert(VisualShaderNodeSwitch::__class_id);
	JS_NewClass(JS_GetRuntime(ctx), VisualShaderNodeSwitch::__class_id, &visual_shader_node_switch_class_def);

	JSValue proto = JS_NewObjectClass(ctx, VisualShaderNodeSwitch::__class_id);
	...
	constructors[VisualShaderNodeSwitch::__class_id] = ctor;
...
void register_visual_shader_node_switch() {
	VisualShaderNodeSwitch::__init_js_class_id();
