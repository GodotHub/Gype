...
static void visual_shader_node_determinant_class_finalizer(JSRuntime *rt, JSValue val) {
	VisualShaderNodeDeterminant *visual_shader_node_determinant = static_cast<VisualShaderNodeDeterminant *>(JS_GetOpaque(val, VisualShaderNodeDeterminant::__class_id));
	if (visual_shader_node_determinant)
		memdelete(visual_shader_node_determinant);
}
...
static JSValue visual_shader_node_determinant_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	VisualShaderNodeDeterminant *visual_shader_node_determinant_class;
	if (argc == 1) 
		visual_shader_node_determinant_class = static_cast<VisualShaderNodeDeterminant *>(Variant(*argv).operator Object *());
	else 
		visual_shader_node_determinant_class = memnew(VisualShaderNodeDeterminant);
...
static int js_visual_shader_node_determinant_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	constructors[VisualShaderNodeDeterminant::__class_id] = ctor;
