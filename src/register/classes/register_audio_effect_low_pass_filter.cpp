...
static JSClassDef audio_effect_low_pass_filter_class_def = {
	...
	.finalizer = audio_effect_low_pass_filter_class_finalizer
};
...
static JSValue audio_effect_low_pass_filter_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	JSValue obj = JS_NewObjectProtoClass(ctx, proto, AudioEffectLowPassFilter::__class_id);
	if (JS_IsException(obj))
		return obj;

	AudioEffectLowPassFilter *audio_effect_low_pass_filter_class;
	if (argc == 1) 
		audio_effect_low_pass_filter_class = static_cast<AudioEffectLowPassFilter *>(Variant(*argv).operator Object *());
	else 
		audio_effect_low_pass_filter_class = memnew(AudioEffectLowPassFilter);
	if (!audio_effect_low_pass_filter_class) {
		JS_FreeValue(ctx, obj);
		return JS_EXCEPTION;
	}
	JS_SetOpaque(obj, audio_effect_low_pass_filter_class);
	return obj;
}
...
static void define_audio_effect_low_pass_filter_enum(JSContext *ctx, JSValue proto) {
}

static int js_audio_effect_low_pass_filter_class_init(JSContext *ctx, JSModuleDef *m) {
	
	classes["AudioEffectLowPassFilter"] = AudioEffectLowPassFilter::__class_id;
	class_id_list.insert(AudioEffectLowPassFilter::__class_id);
	JS_NewClass(JS_GetRuntime(ctx), AudioEffectLowPassFilter::__class_id, &audio_effect_low_pass_filter_class_def);

	JSValue proto = JS_NewObjectClass(ctx, AudioEffectLowPassFilter::__class_id);
	JSValue base_class = JS_GetClassProto(ctx, AudioEffectFilter::__class_id);
	JS_SetPrototype(ctx, proto, base_class);
	JS_SetClassProto(ctx, AudioEffectLowPassFilter::__class_id, proto);

	define_audio_effect_low_pass_filter_property(ctx, proto);
	define_audio_effect_low_pass_filter_enum(ctx, proto);
	...
	constructors[AudioEffectLowPassFilter::__class_id] = ctor;
...
void register_audio_effect_low_pass_filter() {
	AudioEffectLowPassFilter::__init_js_class_id();
	js_init_audio_effect_low_pass_filter_module(ctx);
}