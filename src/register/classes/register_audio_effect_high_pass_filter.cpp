...
static void audio_effect_high_pass_filter_class_finalizer(JSRuntime *rt, JSValue val) {
	AudioEffectHighPassFilter *audio_effect_high_pass_filter = static_cast<AudioEffectHighPassFilter *>(JS_GetOpaque(val, AudioEffectHighPassFilter::__class_id));
	if (audio_effect_high_pass_filter)
		AudioEffectHighPassFilter::free(nullptr, audio_effect_high_pass_filter);
}
...
static JSValue audio_effect_high_pass_filter_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	JSValue obj = JS_NewObjectClass(ctx, AudioEffectHighPassFilter::__class_id);
...
void define_audio_effect_high_pass_filter_property(JSContext *ctx, JSValue obj) {
}

static int js_audio_effect_high_pass_filter_class_init(JSContext *ctx, JSModuleDef *m) {
	
	JS_NewClassID(&AudioEffectHighPassFilter::__class_id);
