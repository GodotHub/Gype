...
static JSValue audio_stream_synchronized_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		audio_stream_synchronized_class = static_cast<AudioStreamSynchronized *>(Variant(*argv).operator Object *());
	else 
...
static void define_audio_stream_synchronized_enum(JSContext *ctx, JSValue proto) {
}
...
static int js_audio_stream_synchronized_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	define_audio_stream_synchronized_enum(ctx, proto);
	...
	constructors[AudioStreamSynchronized::__class_id] = ctor;
