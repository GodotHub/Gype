...
static void visual_shader_node_sample3d_class_finalizer(JSRuntime *rt, JSValue val) {
	...
	if (visual_shader_node_sample3d)
		VisualShaderNodeSample3D::free(nullptr, visual_shader_node_sample3d);
}
...
static JSValue visual_shader_node_sample3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	VisualShaderNodeSample3D *visual_shader_node_sample3d_class;
	JSValue obj = JS_NewObjectClass(ctx, VisualShaderNodeSample3D::__class_id);
	if (JS_IsException(obj))
		return obj;
	visual_shader_node_sample3d_class = memnew(VisualShaderNodeSample3D);
...
static JSValue visual_shader_node_sample3d_class_set_source(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	call_builtin_method_no_ret(&VisualShaderNodeSample3D::set_source, VisualShaderNodeSample3D::__class_id, ctx, this_val, argv);
	return JS_UNDEFINED;
};
static JSValue visual_shader_node_sample3d_class_get_source(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv) {
	return call_builtin_const_method_ret(&VisualShaderNodeSample3D::get_source, VisualShaderNodeSample3D::__class_id, ctx, this_val, argv);
};
