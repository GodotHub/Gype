...
static void separation_ray_shape3d_class_finalizer(JSRuntime *rt, JSValue val) {
	...
	if (separation_ray_shape3d)
		memdelete(separation_ray_shape3d);
}
...
static JSValue separation_ray_shape3d_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	JSValue obj = JS_NewObjectProtoClass(ctx, proto, SeparationRayShape3D::__class_id);
	if (JS_IsException(obj))
		return obj;
	SeparationRayShape3D *separation_ray_shape3d_class = memnew(SeparationRayShape3D);
...
void define_separation_ray_shape3d_property(JSContext *ctx, JSValue obj) {
    JS_DefinePropertyGetSet(
        ...
        JS_NewCFunction(ctx, separation_ray_shape3d_class_set_length, "set_length", 0),
    ...
    JS_DefinePropertyGetSet(
        ...
        JS_NewCFunction(ctx, separation_ray_shape3d_class_set_slide_on_slope, "set_slide_on_slope", 0),
        JS_PROP_CONFIGURABLE | JS_PROP_ENUMERABLE
    );
}
...
static int js_separation_ray_shape3d_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	JSValue proto = JS_NewObjectClass(ctx, SeparationRayShape3D::__class_id);
...
void register_separation_ray_shape3d() {
	SeparationRayShape3D::__init_js_class_id();
