...
static JSValue visual_shader_node_particle_ring_emitter_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (argc == 1) 
		visual_shader_node_particle_ring_emitter_class = static_cast<VisualShaderNodeParticleRingEmitter *>(Variant(*argv).operator Object *());
	else 
...
static int js_visual_shader_node_particle_ring_emitter_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	class_id_list.insert(VisualShaderNodeParticleRingEmitter::__class_id);
	...
	JSValue proto = JS_NewObjectClass(ctx, VisualShaderNodeParticleRingEmitter::__class_id);
	...
	constructors[VisualShaderNodeParticleRingEmitter::__class_id] = ctor;
...
void register_visual_shader_node_particle_ring_emitter() {
	VisualShaderNodeParticleRingEmitter::__init_js_class_id();
