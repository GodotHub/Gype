...
static void audio_stream_playlist_class_finalizer(JSRuntime *rt, JSValue val) {
	...
	if (audio_stream_playlist)
		memdelete(audio_stream_playlist);
}
...
static JSValue audio_stream_playlist_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (JS_IsException(obj))
		return obj;
	AudioStreamPlaylist *audio_stream_playlist_class = memnew(AudioStreamPlaylist);
...
void define_audio_stream_playlist_property(JSContext *ctx, JSValue obj) {
    JS_DefinePropertyGetSet(
        ...
        JS_NewCFunction(ctx, audio_stream_playlist_class_set_shuffle, "set_shuffle", 0),
    ...
    JS_DefinePropertyGetSet(
        ...
        JS_NewCFunction(ctx, audio_stream_playlist_class_set_loop, "set_loop", 0),
        JS_PROP_CONFIGURABLE | JS_PROP_ENUMERABLE
    );
    JS_DefinePropertyGetSet(
        ctx,
        ...
        JS_NewCFunction(ctx, audio_stream_playlist_class_set_fade_time, "set_fade_time", 0),
        JS_PROP_CONFIGURABLE | JS_PROP_ENUMERABLE
    );
    JS_DefinePropertyGetSet(
        ctx,
        ...
        JS_NewCFunction(ctx, audio_stream_playlist_class_set_stream_count, "set_stream_count", 0),
        JS_PROP_CONFIGURABLE | JS_PROP_ENUMERABLE
    );
    JS_DefinePropertyGetSet(
        ctx,
        ...
        JS_NewCFunction(ctx, audio_stream_playlist_class_set_list_stream, "set_list_stream", 0),
        JS_PROP_CONFIGURABLE | JS_PROP_ENUMERABLE
    );
    JS_DefinePropertyGetSet(
        ctx,
