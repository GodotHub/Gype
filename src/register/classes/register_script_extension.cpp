...
static JSValue script_extension_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	JSValue obj = JS_NewObjectProtoClass(ctx, proto, ScriptExtension::__class_id);
	if (JS_IsException(obj))
		return obj;

	ScriptExtension *script_extension_class;
	if (argc == 1) 
		script_extension_class = static_cast<ScriptExtension *>(Variant(*argv).operator Object *());
	else 
...
static int js_script_extension_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	JSValue proto = JS_NewObjectClass(ctx, ScriptExtension::__class_id);
	...
	constructors[ScriptExtension::__class_id] = ctor;
...
void register_script_extension() {
	ScriptExtension::__init_js_class_id();
