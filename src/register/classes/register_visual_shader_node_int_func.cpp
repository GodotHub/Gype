...
static JSValue visual_shader_node_int_func_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	VisualShaderNodeIntFunc *visual_shader_node_int_func_class;
	if (argc == 1) 
		visual_shader_node_int_func_class = static_cast<VisualShaderNodeIntFunc *>(Variant(*argv).operator Object *());
	else 
		visual_shader_node_int_func_class = memnew(VisualShaderNodeIntFunc);
...
static int js_visual_shader_node_int_func_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	constructors[VisualShaderNodeIntFunc::__class_id] = ctor;
