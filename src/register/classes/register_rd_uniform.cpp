...
static JSValue rd_uniform_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	if (JS_IsException(obj))
		return obj;
	RDUniform *rd_uniform_class = memnew(RDUniform);
	if (!rd_uniform_class) {
...
void define_rd_uniform_property(JSContext *ctx, JSValue obj) {
    JS_DefinePropertyGetSet(
        ...
        JS_NewCFunction(ctx, rd_uniform_class_set_uniform_type, "set_uniform_type", 0),
    ...
    JS_DefinePropertyGetSet(
        ...
        JS_NewCFunction(ctx, rd_uniform_class_set_binding, "set_binding", 0),
        JS_PROP_CONFIGURABLE | JS_PROP_ENUMERABLE
    );
}
