...
static void visual_shader_node_color_op_class_finalizer(JSRuntime *rt, JSValue val) {
	VisualShaderNodeColorOp *visual_shader_node_color_op = static_cast<VisualShaderNodeColorOp *>(JS_GetOpaque(val, VisualShaderNodeColorOp::__class_id));
	if (visual_shader_node_color_op)
		memdelete(visual_shader_node_color_op);
}
...
static JSValue visual_shader_node_color_op_class_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
	...
	VisualShaderNodeColorOp *visual_shader_node_color_op_class;
	if (argc == 1) 
		visual_shader_node_color_op_class = static_cast<VisualShaderNodeColorOp *>(Variant(*argv).operator Object *());
	else 
		visual_shader_node_color_op_class = memnew(VisualShaderNodeColorOp);
...
static int js_visual_shader_node_color_op_class_init(JSContext *ctx, JSModuleDef *m) {
	...
	constructors[VisualShaderNodeColorOp::__class_id] = ctor;
