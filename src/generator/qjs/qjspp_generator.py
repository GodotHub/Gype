...
from binding_generator import is_pod_type, is_included_struct_type, is_packed_array
...
def list_files_relative(directory):
    ...
    ''.count('visual_shader')
    return filter(lambda path: path.count('visual_shader_node') == 0, map(lambda path: str(path).replace('\\', '/'), relative_paths))
...
def generate_qjspp_h(env, data):
    ...
    with open(f'{root}/include/qjs/qjspp.h', 'w') as file:
        file.write(render)
...
def generate_godot_headers_h(env):
    ...
    with open(f'{root}/include/qjs/godot_headers.h', 'w') as file:
        file.write(render)
