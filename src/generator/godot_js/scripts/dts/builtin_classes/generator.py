...
def gen_builtin_classes_d_ts():
    env = Environment(loader=FileSystemLoader(searchpath='./templates/dts/builtin_classes'))
    cpp_template = env.get_template('builtin_classes.d.ts.jinja')
    gen_path = Path.joinpath(generated_root_dir,'example','addons','gype','godot','builtin_classes')
    config_env(env)
    for clazz in gde_json['builtin_classes']:
