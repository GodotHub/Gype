...
// template <typename T>
// JSValue convert_to_js(JSContext *ctx, T arg) {
// 	throw _excep;
// Variant variant(arg);
// switch (variant.get_type()) {
// 	case Variant::Type::INT:
// 		return JS_NewInt64(ctx, variant);
...
// 		return JS_NewString(ctx, std::string(String(variant).utf8().get_data()).c_str());
// 	case Variant::Type::ARRAY: {
// 		const Array &arr = variant;
...
template <typename T>
typename std::enable_if<std::is_fundamental_v<T>, JSValue>
...
// template <typename T>
// typename std::enable_if<std::is_same<>, JSValue>
// convert_to_js(JSContext *ctx, T arg){}

template <typename T>
typename std::enable_if<std::is_base_of_v<Object, std::remove_pointer_t<T>>, JSValue>::type
convert_to_js(JSContext *ctx, T arg) {
	JSValue obj = JS_NewObjectClass(ctx, T::__class_id);
	gd_set_wrapper(ctx, obj, gd_new_JSObject_wrapper(ctx, obj));
	return obj;
}

template <typename Tuple, std::size_t... I>
void call_js_function_with_tuple_impl(JSContext *ctx, JSValue func_obj, JSValue this_obj, const Tuple &tuple, std::index_sequence<I...>) {
	JS_Call(ctx, func_obj, this_obj, sizeof...(I), { convert_to_js(ctx, std::get<I>(tuple))... });
}

template <typename Tuple>
void call_js_function_with_tuple(JSContext *ctx, JSValue func_obj, JSValue this_obj, const Tuple &tuple) {
	call_js_function_with_tuple_impl(ctx, func_obj, this_obj, tuple, std::make_index_sequence<std::tuple_size<Tuple>::value>{});
}
