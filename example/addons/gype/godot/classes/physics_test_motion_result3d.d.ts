
import { RefCounted } from "@godot/classes/ref_counted";
import { GodotObject } from "@godot/classes/godot_object";
export declare class PhysicsTestMotionResult3D extends RefCounted{
  public get_travel(): Vector3;
  public get_remainder(): Vector3;
  public get_collision_safe_fraction(): number;
  public get_collision_unsafe_fraction(): number;
  public get_collision_count(): number;
  public get_collision_point(_collision_index: number = 0): Vector3;
  public get_collision_normal(_collision_index: number = 0): Vector3;
  public get_collider_velocity(_collision_index: number = 0): Vector3;
  public get_collider_id(_collision_index: number = 0): number;
  public get_collider_rid(_collision_index: number = 0): RID;
  public get_collider(_collision_index: number = 0): GodotObject;
  public get_collider_shape(_collision_index: number = 0): number;
  public get_collision_local_shape(_collision_index: number = 0): number;
  public get_collision_depth(_collision_index: number = 0): number;
}