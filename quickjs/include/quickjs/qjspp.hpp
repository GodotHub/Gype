...
GDExtensionInt encodeInt(JSContext *ctx, JSValue v);
...
// template <>
// struct js_traits<godot::PropertyInfo> {
// 	static JSValue wrap(JSContext *ctx, godot::PropertyInfo value) {
// 		return js_traits<std::shared_ptr<godot::PropertyInfo>>::wrap(ctx, std::shared_ptr<godot::PropertyInfo>(&value));
// 	}

// 	static godot::PropertyInfo unwrap(JSContext *ctx, JSValue value) {
// 		return *js_traits<std::shared_ptr<godot::PropertyInfo>>::unwrap(ctx, value).get();
// 	}
// };
...
// template <>
// struct js_traits<GDExtensionPropertyInfo> {
// 	static JSValue wrap(JSContext *ctx, GDExtensionPropertyInfo value) {
// 		return js_traits<std::shared_ptr<GDExtensionPropertyInfo>>::wrap(ctx, std::shared_ptr<GDExtensionPropertyInfo>(&value));
// 	}

// 	static GDExtensionPropertyInfo unwrap(JSContext *ctx, JSValue value) {
// 		return *js_traits<std::shared_ptr<GDExtensionPropertyInfo>>::unwrap(ctx, value).get();
// 	}
// };
...
// struct js_traits<std::unordered_map<std::variant<K>, std::variant<V>>> {
// 	static JSValue wrap(JSContext *ctx, std::unordered_map<std::variant<K>, std::variant<V>> value) {
// 		JSValue js_map = JS_NewObjectClass(ctx, 38);
// 		JSValue setter = JS_GetPropertyStr(ctx, js_map, "set");
// 		for (const std::pair<std::variant<K>, std::variant<V>> &pair : value) {
// 			JSValue arr[2] = { Value(ctx, pair.first), Value(ctx, pair.second) };
// 			JS_Call(ctx, setter, js_map, 2, arr);
// 		}
// 		return js_map;
// 	}

// 	static std::unordered_map<std::variant<K>, std::variant<V>> unwrap(JSContext *ctx, JSValue value) {
// 		std::unordered_map<std::variant<K>, std::variant<V>> map;
// 		JSValue keys_func = JS_GetPropertyStr(ctx, value, "keys");
// 		JSValue keys = JS_Call(ctx, keys_func, value, 0, NULL);
// 		JSValue get_func = JS_GetPropertyStr(ctx, value, "get");
// 		int length = js_traits<int>::unwrap(ctx, JS_GetPropertyStr(ctx, keys, "length"));
// 		for (int i = 0; i < length; i++) {
// 			JSValue js_k = JS_GetPropertyUint32(ctx, keys, i);
// 			JSValue js_v = JS_Call(ctx, get_func, value, 1, &js_k);
// 			K k = js_traits<std::variant<K>>::unwrap(ctx, js_k);
// 			V v = js_traits<std::variant<V>>::unwrap(ctx, js_v);
// 			map[k] = (v);
// 		}
// 		return map;
// 	}
...
// template <>
// struct js_traits<std::unordered_map<std::variant<std::string>, godot::PropertyInfo::Type>> {
// 	static JSValue wrap(JSContext *ctx, std::unordered_map<std::variant<std::string>, godot::PropertyInfo::Type> value) {
// 		JSValue js_map = JS_NewObjectClass(ctx, 38);
// 		JSValue setter = JS_GetPropertyStr(ctx, js_map, "set");
...
// 	static std::unordered_map<std::variant<std::string>, godot::PropertyInfo::Type> unwrap(JSContext *ctx, JSValue value) {
// 		std::unordered_map<std::variant<std::string>, godot::PropertyInfo::Type> map;
// 		JSValue keys_func = JS_GetPropertyStr(ctx, value, "keys");
// 		JSValue keys = JS_Call(ctx, keys_func, value, 0, NULL);
// 		JSValue get_func = JS_GetPropertyStr(ctx, value, "get");
// 		int length = js_traits<int>::unwrap(ctx, JS_GetPropertyStr(ctx, keys, "length"));
...
// 			JSValue js_k = JS_GetPropertyUint32(ctx, keys, i);
// 			JSValue js_v = JS_Call(ctx, get_func, value, 1, &js_k);
// 			std::variant<std::string> k = js_traits<std::variant<std::string>>::unwrap(ctx, js_k);
// 			godot::PropertyInfo::Type v = js_traits<godot::PropertyInfo::Type>::unwrap(ctx, js_v);
// 			map[k] = (v);
// 		}
// 		return map;
// 	}
...
inline uint8_t *get_typed_array_buf(JSContext *ctx, JSValue v) {
...
// static uint8_t (*get_variant_native_ptr(JSContext *ctx, JSValue v))[GODOT_CPP_VARIANT_SIZE] {
// 	uint8_t *buf = get_typed_array_buf(ctx, v);
// 	if (buf) {
// 		return reinterpret_cast<uint8_t(*)[GODOT_CPP_VARIANT_SIZE]>(buf);
// 	}
// 	return nullptr;
// }

// 获取参数数组的指针
inline std::vector<void *> get_args(JSContext *ctx, JSValue v) {
	...
	for (int i = 0; i < length; i++) {
		...
		} else if (JS_IsBool(el)) {
			bool b = encodeBool(ctx, el);
			bool *m_b = reinterpret_cast<bool *>(js_malloc(ctx, sizeof(bool)));
			*m_b = b;
			args.push_back(m_b);
		}
	}
...
inline std::vector<void *> get_args(JSContext *ctx, std::vector<JSValue> v) {
	int length = v.size();
	if (length == 0) {
		return std::vector<void *>(0);
	}
	...
	for (int i = 0; i < length; i++) {
		JSValue el = v[i];
		if (JS_IsObject(el) && is_typed_array(el)) {
			args.push_back(get_typed_array_buf(ctx, el));
		} else if (JS_IsNumber(el)) {
			if (is_int(el)) {
				int64_t num = encodeInt(ctx, el);
			...
			bool v = JS_ToBool(ctx, el);
			bool *m_v = reinterpret_cast<bool *>(js_malloc(ctx, sizeof(bool)));
			*m_v = v;
			args.push_back(m_v);
		}
	}
...
inline bool encodeBool(JSContext *ctx, JSValue v) {
	if (JS_IsBool(v)) {
		return (bool)JS_ToBool(ctx, v);
	}
	return false;
}

inline GDExtensionInt encodeInt(JSContext *ctx, JSValue v) {
	GDExtensionInt i;
...
inline double encodeDouble(JSContext *ctx, JSValue v) {
	double d;
...
inline int variant_size(int type) {
	switch (type) {
...
// static JSValue to_js_value(JSContext *ctx, GDExtensionVariantType type, const godot::Variant &value) {
// 	switch (type) {
// 		case GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_INT:
...
inline const char *to_js_type(int type) {
	switch (type) {
		case GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_NIL:
			return "nil";
		case GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_INT:
...
// static JSValue to_js_value(JSContext *ctx, int type, uint8_t *value) {
...
// 	JSValue *args = new JSValue[3]{ buf, JS_NewInt64(ctx, 0), JS_NewInt64(ctx, len) };
// 	JSValue opaque = JS_NewTypedArray(ctx, len, args, JSTypedArrayEnum::JS_TYPED_ARRAY_UINT8);
// 	JS_SetPropertyStr(ctx, ret, "opaque", opaque);
// 	return ret;
// }

inline JSValue to_js_variant(JSContext *ctx, int type, JSValue opaque) {
	...
	JSValue ret = JS_GetPropertyStr(ctx, js_variant, "opaque");
	return ret;
}
...
// static uint8_t *u8ptr_cast_to(JSContext *ctx, GDExtensionVariantType type, uint8_t *u8_buf) {
// 	void *ret;
// 	switch (type) {
// 		case GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_NIL:
// 			ret = godot::Variant()._native_ptr();
// 			break;
// 		case GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_INT:
// 			ret = godot::Variant(*(int *)u8_buf)._native_ptr();
// 			break;
// 		case GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_FLOAT:
// 			ret = godot::Variant(*(double *)u8_buf)._native_ptr();
// 			break;
// 		case GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_BOOL:
// 			ret = godot::Variant(*(bool *)u8_buf)._native_ptr();
// 			break;
// 		default: {
// 			JSValue value = to_js_value(ctx, (int)type, reinterpret_cast<void *>(u8_buf));
// 		} break;
// 	}
// 	return (uint8_t *)ret;
// }

inline void *to_c_value(JSContext *ctx, JSValue value) {
	void *arg = nullptr;
	...
	if (is_typed_array(value)) {
		...
		GDExtensionBool ret = encodeBool(ctx, value);
		arg = js_mallocz(ctx, sizeof(GDExtensionBool));
		*reinterpret_cast<GDExtensionBool *>(arg) = ret;
		...
		*reinterpret_cast<double *>(arg) = ret;
		...
		godot::Variant variant = godot::Variant();
		uint8_t *buf = *(uint8_t(*)[GODOT_CPP_VARIANT_SIZE])variant._native_ptr();
		uint8_t *cpy = new uint8_t[GODOT_CPP_VARIANT_SIZE];
		memcpy(cpy, buf, GODOT_CPP_VARIANT_SIZE);
		return cpy;
	}
...
inline void *malloc_variant(JSContext *ctx, GDExtensionVariantType type) {
	switch (type) {
		case GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_NIL:
			return memnew(godot::Variant)->_native_ptr();
		case GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_BOOL:
			return memalloc(1);
		case GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_INT:
		case GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_FLOAT:
		...
		case GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_RID:
		case GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_DICTIONARY:
			return memalloc(8 * sizeof(uint8_t));
		case GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_VECTOR3:
		case GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_VECTOR3I:
			return memalloc(12 * sizeof(uint8_t));
		case GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_RECT2:
		case GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_RECT2I:
		case GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_VECTOR4:
		case GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_VECTOR4I:
		...
		case GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_PACKED_VECTOR4_ARRAY:
		case GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_PACKED_COLOR_ARRAY:
			return memalloc(16 * sizeof(uint8_t));
		case GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_TRANSFORM2D:
		case GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_VARIANT_MAX:
			return memalloc(24 * sizeof(uint8_t));
		case GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_BASIS:
			return memalloc(36 * sizeof(uint8_t));
		case GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_TRANSFORM3D:
			return memalloc(48 * sizeof(uint8_t));
		case GDExtensionVariantType::GDEXTENSION_VARIANT_TYPE_PROJECTION:
			return memalloc(64 * sizeof(uint8_t));
		default:
			return nullptr;
	}
}

inline void *to_c_pointer(JSContext *ctx, Pointer js_pointer) {
	std::shared_ptr<JSPointer> pointer = js_traits<std::shared_ptr<JSPointer>>::unwrap(ctx, js_pointer);
	return reinterpret_cast<void *>(pointer->get_pointer());
}
...
struct js_traits<GDExtensionPtrUtilityFunction> {
	static JSValue inner_method(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv, int magic, JSValue *func_data) {
		...
		const GDExtensionConstTypePtr *p_args = args.data();
		int p_argument_count = args.size();
		auto func = js_traits<std::function<void(GDExtensionTypePtr, const GDExtensionConstTypePtr *, int)>>::unwrap(ctx, *func_data);
		func(r_return, p_args, p_argument_count);
		return JS_UNDEFINED;
	}
...
struct js_traits<GDExtensionVariantFromTypeConstructorFunc> {
	static JSValue inner_method(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv, int magic, JSValue *func_data) {
		...
		GDExtensionTypePtr arg2 = to_c_value(ctx, argv[1]);
		auto func = js_traits<void(GDExtensionUninitializedVariantPtr, GDExtensionTypePtr)>::unwrap(ctx, *func_data);
		func(arg1, arg2);
		return JS_UNDEFINED;
	}
...
// template <>
// struct js_traits<GDExtensionInstanceBindingCreateCallback> {
// 	static JSValue inner_method(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv, int magic, JSValue *func_data) {
// 		void *p_token = (void *)js_traits<std::shared_ptr<JSPointer>>::unwrap(ctx, argv[0])->get_pointer();
// 		void *p_instance = (void *)js_traits<std::shared_ptr<JSPointer>>::unwrap(ctx, argv[1])->get_pointer();
// 		auto method = js_traits<std::function<void *(void *, void *)>>::unwrap(ctx, *func_data);
// 		return js_traits<std::shared_ptr<JSPointer>>::wrap(ctx, (intptr_t)method(p_token, p_instance));
// 	}

// 	static JSValue wrap(JSContext *ctx, GDExtensionInstanceBindingCreateCallback method) noexcept {
// 		JSValue data = js_traits<std::function<void *(void *, void *)>>::wrap(ctx, method);
// 		return JS_NewCFunctionData(ctx, &inner_method, 2, 0, 1, &data);
// 	}

// 	static GDExtensionInstanceBindingCreateCallback unwrap(JSContext *ctx, JSValue method) {
// 		std::function<void *(void *, void *)> func = js_traits<std::function<void *(void *, void *)>>::unwrap(ctx, method);
// 		GDExtensionInstanceBindingCreateCallback method_ptr = func.target<void *(void *, void *)>();
// 		return method_ptr;
// 	}
// };

// template <>
// struct js_traits<GDExtensionInstanceBindingFreeCallback> {
// 	static JSValue inner_method(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv, int magic, JSValue *func_data) {
// 		void *p_token = (void *)js_traits<std::shared_ptr<JSPointer>>::unwrap(ctx, argv[0])->get_pointer();
// 		void *p_instance = (void *)js_traits<std::shared_ptr<JSPointer>>::unwrap(ctx, argv[1])->get_pointer();
// 		void *p_binding = (void *)js_traits<std::shared_ptr<JSPointer>>::unwrap(ctx, argv[2])->get_pointer();
// 		auto method = js_traits<std::function<void(void *, void *, void *)>>::unwrap(ctx, *func_data);
// 		method(p_token, p_instance, p_binding);
// 		return JS_UNDEFINED;
// 	}

// 	static JSValue wrap(JSContext *ctx, GDExtensionInstanceBindingFreeCallback method) noexcept {
// 		JSValue data = js_traits<std::function<void(void *, void *, void *)>>::wrap(ctx, method);
// 		return JS_NewCFunctionData(ctx, &inner_method, 3, 0, 1, &data);
// 	}

// 	static GDExtensionInstanceBindingFreeCallback unwrap(JSContext *ctx, JSValue method) {
// 		std::function<void(void *, void *, void *)> func = js_traits<std::function<void(void *, void *, void *)>>::unwrap(ctx, method);
// 		GDExtensionInstanceBindingFreeCallback method_ptr = func.target<void(void *, void *, void *)>();
// 		return method_ptr;
// 	}
// };

// template <>
// struct js_traits<GDExtensionInstanceBindingReferenceCallback> {
// 	static JSValue inner_method(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv, int magic, JSValue *func_data) {
// 		void *p_token = (void *)js_traits<std::shared_ptr<JSPointer>>::unwrap(ctx, argv[0])->get_pointer();
// 		void *p_instance = (void *)js_traits<std::shared_ptr<JSPointer>>::unwrap(ctx, argv[1])->get_pointer();
// 		GDExtensionBool p_reference = js_traits<GDExtensionBool>::unwrap(ctx, argv[2]);
// 		auto method = js_traits<std::function<GDExtensionBool(void *, void *, GDExtensionBool)>>::unwrap(ctx, *func_data);
// 		method(p_token, p_instance, p_reference);
// 		return JS_UNDEFINED;
// 	}

// 	static JSValue wrap(JSContext *ctx, GDExtensionInstanceBindingReferenceCallback method) noexcept {
// 		JSValue data = js_traits<std::function<GDExtensionBool(void *, void *, GDExtensionBool)>>::wrap(ctx, method);
// 		return JS_NewCFunctionData(ctx, &inner_method, 3, 0, 1, &data);
// 	}

// 	static GDExtensionInstanceBindingReferenceCallback unwrap(JSContext *ctx, JSValue method) {
// 		std::function<GDExtensionBool(void *, void *, GDExtensionBool)> func = js_traits<std::function<GDExtensionBool(void *, void *, GDExtensionBool)>>::unwrap(ctx, method);
// 		GDExtensionInstanceBindingReferenceCallback method_ptr = func.target<GDExtensionBool(void *, void *, GDExtensionBool)>();
// 		return method_ptr;
// 	}
// };

template <>
struct js_traits<GDExtensionClassCreateInstance> {
	static JSValue inner_method(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv, int magic, JSValue *func_data) {
		auto method = js_traits<std::function<void *(void *)>>::unwrap(ctx, *func_data);
		void *ret = method(argv);
		return *reinterpret_cast<JSValue *>(ret);
	}

	static JSValue wrap(JSContext *ctx, GDExtensionClassCreateInstance method) {
		JSValue data = js_traits<std::function<void *(void *)>>::wrap(ctx, method);
		return JS_NewCFunctionData(ctx, &inner_method, 1, 0, 1, &data);
	}
};
...
inline bool is_exception(JSContext *ctx, JSValue exp) {
