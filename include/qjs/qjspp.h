...
struct js_traits<unsigned char> {
	static JSValue wrap(JSContext *ctx, unsigned char value) {
		return JS_NewBool(ctx, static_cast<uint8_t>(value));
	}
...
struct js_traits<unsigned char *> {
	static JSValue wrap(JSContext *ctx, unsigned char *value) {
		return JS_NewBool(ctx, *value ? true : false);
	}

	static unsigned char *unwrap(JSContext *ctx, JSValueConst value) {
		if (JS_IsBool(value)) {
			bool boolValue = JS_ToBool(ctx, value);
			return new unsigned char{ static_cast<unsigned char>(boolValue) };
		} else {
			throw exception{ ctx };
		}
	}
...
typedef void (*Function_1)(void *, void *);

template <>
struct js_traits<Function_1> {
	static JSValue _func(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv, int magic, JSValue *func_data) {
		auto func = reinterpret_cast<Function_1>(JS_GetOpaque2(ctx, *func_data, 15));
		func(js_traits<void *>::unwrap(ctx, argv[0]), js_traits<void *>::unwrap(ctx, argv[1]));
		return JS_UNDEFINED;
	}

	static JSValue wrap(JSContext *ctx, Function_1 func) {
		JSValue func_body = JS_NewObjectClass(ctx, 15);
		JS_SetOpaque(func_body, (void *)func);
		return JS_NewCFunctionData(ctx, &_func, 2, 0, 1, &func_body);
	}
};

typedef void (*Function_2)(const void *, const void *, void *);

template <>
struct js_traits<Function_2> {
	static JSValue _func(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv, int magic, JSValue *func_data) {
		auto func = reinterpret_cast<Function_2>(JS_GetOpaque2(ctx, *func_data, 15));
		func(js_traits<const void *>::unwrap(ctx, argv[0]), js_traits<const void *>::unwrap(ctx, argv[1]), js_traits<void *>::unwrap(ctx, argv[2]));
		return JS_UNDEFINED;
	}

	static JSValue wrap(JSContext *ctx, Function_2 func) {
		JSValue func_body = JS_NewObjectClass(ctx, 15);
		JS_SetOpaque(func_body, (void *)func);
		return JS_NewCFunctionData(ctx, &_func, 3, 0, 1, &func_body);
	}
};

typedef void (*Function_3)(void *, const void *const *, void *, int);

template <>
struct js_traits<Function_3> {
	static JSValue _func(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv, int magic, JSValue *func_data) {
		auto func = reinterpret_cast<Function_3>(JS_GetOpaque2(ctx, *func_data, 15));
		func(js_traits<void *>::unwrap(ctx, argv[0]),
				js_traits<const void *const *>::unwrap(ctx, argv[1]),
				js_traits<void *>::unwrap(ctx, argv[2]),
				js_traits<int>::unwrap(ctx, argv[3]));
		return JS_UNDEFINED;
	}

	static JSValue wrap(JSContext *ctx, Function_3 func) {
		JSValue func_body = JS_NewObjectClass(ctx, 15);
		JS_SetOpaque(func_body, (void *)func);
		return JS_NewCFunctionData(ctx, &_func, 4, 0, 1, &func_body);
	}
};

typedef void (*Function_4)(void *, const void *const *);

template <>
struct js_traits<Function_4> {
	static JSValue _func(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv, int magic, JSValue *func_data) {
		auto func = reinterpret_cast<Function_4>(JS_GetOpaque2(ctx, *func_data, 15));
		func(js_traits<void *>::unwrap(ctx, argv[0]),
				js_traits<const void *const *>::unwrap(ctx, argv[1]));
		return JS_UNDEFINED;
	}

	static JSValue wrap(JSContext *ctx, Function_4 func) {
		JSValue func_body = JS_NewObjectClass(ctx, 15);
		JS_SetOpaque(func_body, (void *)func);
		return JS_NewCFunctionData(ctx, &_func, 2, 0, 1, &func_body);
	}
		...
		switch (JS_VALUE_GET_TAG(v)) {
			...
			case JS_TAG_OBJECT:
				...
				if constexpr (is_shared_ptr<T>::value) {
					...
					if (class_id == js_traits<T>::QJSClassId)
						return true;
				}
				return false;
	...
	static std::variant<Ts...> unwrap(JSContext *ctx, JSValueConst v) {
		...
		switch (tag) {
			...
			case JS_TAG_OBJECT:
				...
				JSClassID class_id;
				JS_NewClassID(&class_id);
				if (auto result = unwrapObj<Ts...>(ctx, v, class_id)) {
					return *result;
				}
				// MODIFY
				...
				JS_ThrowTypeError(ctx, "Expected type %s, got object with classid %d",
						QJSPP_TYPENAME(std::variant<Ts...>), class_id);
				break;
...
struct js_traits<std::shared_ptr<T>> {
	...
	static std::shared_ptr<T> unwrap(JSContext *ctx, JSValueConst v) {
		...
		JSClassID obj_class_id;
		JS_NewClassID(&obj_class_id);
...
public:
	...
	std::string toJSON(const Value &replacer = Value::Undefined(nullptr), const Value &space = Value::Undefined(nullptr)) const {
		assert(ctx);
		assert(!replacer.ctx || ctx == replacer.ctx);
		assert(!space.ctx || ctx == space.ctx);
		return (std::string)Value{ ctx, JS_JSONStringify(ctx, v, replacer.v, space.v) };
